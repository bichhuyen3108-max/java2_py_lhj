...
function addNewRowToTable(item){
    ...
    radio.value = item.id; // 상품 ID를 값으로 설정
    ...
    orderInput.min = '1';
    orderInput.id = `orderCount-${item.id}`; // 상품 ID로 고유 ID 부여
    ...
    row.insertCell(6).textContent = item.createdAt;
    row.insertCell(7).textContent = item.updatedAt;

    radio.addEventListener("change", () => {
  document.querySelectorAll(".order-count-input").forEach(inp => {
    inp.disabled = true;
    inp.value = "";
    });
...
async function handleAddItem(event){
    ...
    try {
        await apiFetch('/product/createProduct',{
            method : 'POST',
...
function renderOrderHistoryTable(orders){
    ...
    orders.forEach(order => {
        ...
        row.insertCell(1).textContent = order.pice;
...
function handleRadioChange(event){
    // 모증 주문 개수 입력 필드를 비활성화
    document.querySelector('.order-count-input').
    forEach( input => {
        input.disabled = true;
        input.value ='';
    });

    //선택된 라디오버튼의 상품 ID를 거져온다
    ...
    //주문하기
     async function handleOrder() {
        ...
        if(!selectedRadio){
            orderMessageElement.textContent ='주문할 상품을 선택하세요.';
            orderHistoryTableBody.style.color ='orange';
            return;
        }
        ...
        const productCount = parseInt(orderInput,10); // 입력헌 개수를 숫자로 변환

        //진짜 숫자를 입력했는지, 음수를 입력했는지 조사
        if(isNaN(productCount) || productCount <0){
            orderMessageElement.textContent ='우호한 주문 개수를 입력해주세요(1이상)';
            orderMessageElement.style.color='orange';
            orderInput.focus(); //입력 바로 해라
            return;
        }
