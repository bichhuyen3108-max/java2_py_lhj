...
public class Ex1_stream {
	...
	//forEach()  각요소에 대해 작업을 수행하는 최종 연산
	
	//중간연산(내가 원하는 걸로 가공)
	List<Integer> nums = List.of(1,2,3,4,5,7,3,5,8,0,1,6,4,2,6,9,2);
	
	//filter - 조건에 맞는 것만 남가는 중간연산
	nums.stream()
		.filter(x -> x%2 ==0)//짝수만
		.forEach(System.out::println);
	
	//map = 요소에 처리를 해주는 중간 연산
	//map: chuyển đổi từng phần tử của stream
	names.stream()
		.map(x -> x + "님")
		.forEach(System.out::println);
	...
	//스트림으로 가공한 결과을 List, Set같은 컬랙션으로 다시 만들고 싶으면
	...
	List<Integer> evenList = nums.stream()
								...
								.distinct()
								.sorted()
								.collect(Collectors.toList());//결과를 List로 반들어서 반환
	
	//Collectors에 들어있는 메서드
	//toList() : 리스트로 반환
	//toSet() : set으로 반환
	//joining(",") : 문자열들을 하나로 합치기
	//groupingBy(...) : 특정 기분으로 그룹 나누기
	
	List<String> items = List.of("A","B","C","A");
	...
	List<String> name2 = List.of("Kim","Lee","Park","Choi","Jin");
	
	Map<Integer, List<String>> grouped = name2.stream()
