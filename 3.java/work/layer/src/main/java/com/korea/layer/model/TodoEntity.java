...
//Entity
//데이터ㄹ 담는 역할과 DB의 티이블과 스키마를 표현하는 역할을 한다
@Builder
@NoArgsConstructor
@AllArgsConstructor
@Data
//자바 클래스를 인티티로 지정하기 위해서 사용된다
//이름을 부여하고 싶다면 @Entity("TodoEntity")처럼
//매개변수를 널을 수 있다
@Entity
@Table(name="Todo") //이 엔티티가 어떤 태이블과 매핑될지를 지정한다
//우리가 Todo태이블을 만들지는 읺았지만 JPA가 엔티티를 보고 테이블을 자동생성해준다
...
public class TodoEntity {
	...
	private String id; //이 갹체의 id
	private String userId; //이 객채를 생성한 유저의 id
